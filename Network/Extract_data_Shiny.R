...
org_nodes <- read.csv2("Network/nodes.csv")

# Extract publication matrix
...
a1 <- cbind(aggregate(mat$Years, list(mat$Years), length)[-c(1,10),],
            aggregate(mat$Years[cond1], list(mat$Years[cond1]), length)[-1,],
            aggregate(mat$Years[cond2], list(mat$Years[cond2]), length)[-c(1,10),],
            aggregate(mat$Years[cond3], list(mat$Years[cond3]), length)[-9,],
            aggregate(mat$Years[cond4], list(mat$Years[cond4]), length)[-c(1,10),])
a1 <- a1[,-c(3,5,7,9)]
...
colnames(tab)[-c(1,2,3)] <- seq(2015,2024,1)
edges[[1]] <- tab
temp[,4] <- 0
...
# Export data
save(edges, nodes, a1, a2, file = "Vizu/Data.Rdata")
